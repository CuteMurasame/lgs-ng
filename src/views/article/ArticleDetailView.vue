...
import { useMessage, NSpace, NButton, NIcon, NTag, NDivider, NGrid, NGi, NSkeleton, NAnchor, NAnchorLink, NTimeline, NTimelineItem } from 'naive-ui';
import {
	ShareSocialOutline, CopyOutline, SyncOutline, TrashOutline,
	ArrowBackOutline, NewspaperOutline, CalendarOutline, ListOutline, TimeOutline
...
const loadHistory = async () => {
	...
	try {
		...
		if (res.data) {
			versionHistory.value = res.data.map(h => ({
				version: h.version,
				...
				<div v-if="article">
					<Card :title="article.title" :icon="NewspaperOutline">
						...
						<n-space>
							...
							<n-button size="small" secondary tag="a"
					...
					<Card title="相关推荐" v-if="recommended.length">
						<div class="article-list">
							<div v-for="it in recommended" :key="it.id" class="article-item">
								<Card :title="it.title" :icon="NewspaperOutline" class="clickable-card"
...
:deep(.n-button--error-type.n-button--ghost-type) {
	border-color: rgba(255, 59, 48, 0.4) !important;
	color: #FF3B30 !important;
}

:deep(.n-button--error-type.n-button--ghost-type:hover) {
